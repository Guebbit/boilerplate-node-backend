#!/usr/bin/env tsx
/**
 * Publish the demo dataset as the API actually serves it — `npm run scenario:build`.
 *
        ...
        if (checkOnly) {
            console.error(
                `[seed-export] STALE — db/demo/demo-data.json does not match what the seeders produce.\n` +
                    `  A factory changed without the dataset being re-exported, or the file was hand-edited.\n` +
                    `  Fix with: npm run scenario:build`
            );
            return 1;
        }
