 ...
 * `@modules/<name>/demo` for its demo fixtures — and `eslint.config.ts` enforces that nothing
 ...
 * something asserts that they carry different traffic; a `/demo` import from a service would
 * otherwise be exactly as invisible as it was before, just spelled differently.
 *
 * So: only a `demo.ts` may take a `/demo` path. Everything else in `src/` may not.
 */
...
/** Every `@modules/<name>/demo` specifier a file imports, however the import is written. */
const readDemoImports = (file: string): string[] =>
    [...readFileSync(file, 'utf8').matchAll(/["']@modules\/([^"'/]+)\/demo["']/g)].map(
        (match) => match[1]
    );
...
describe('the demo entry point', () => {
    ...
    it('is taken by more than nothing, so the assertions below are not vacuous', () => {
        ...
        const takers = sourceFiles.filter((file) => readDemoImports(file).length > 0);
    ...
    it('is imported only by other seeders', () => {
        const violations = sourceFiles
            .filter((file) => path.basename(file) !== 'demo.ts')
            .flatMap((file) =>
                readDemoImports(file).map(
                    (target) =>
                        `${path.relative(SOURCE_ROOT, file)} imports @modules/${target}/demo`
                )
            );
    ...
    it('is never re-exported through a module barrel', () => {
        ...
        // `./demo` puts demo data back on the runtime surface, and every assertion above keeps
        ...
        const leaking = readdirSync(MODULES_ROOT)
            ...
            .filter(({ barrel }) => /from\s+["']\.\/demo["']/.test(readFileSync(barrel, 'utf8')))
            .map(({ name }) => `${name}/index.ts re-exports ./demo`);
