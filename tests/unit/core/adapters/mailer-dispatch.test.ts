/**
 * `enqueueEmail` — the queue-or-send-inline dispatch in `src/core/adapters/mailer.ts`.
 *
...
const TEMPLATE = 'email-reset-request.ejs';
/**
 * Every variable the chosen template interpolates. The inline paths RENDER for real (only the
 * SMTP transport is mocked), so a missing variable is an EJS ReferenceError rather than a
