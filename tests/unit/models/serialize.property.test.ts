/**
 * Property-based tests — `src/models/serialize.ts`.
 *
...
import { applySerialization, type TSerializeTransform } from '@models/serialize';
...
const withReservedFields = () =>
    fc
        ...
        .map(([body, id, version]) => ({
            ...body,
            _id: id,
            // eslint-disable-next-line @typescript-eslint/naming-convention -- Mongoose's key
            __v: version
        }));
...
describe('applySerialization — universal guarantees', () => {
    ...
    it('returns the same object it was handed, mutated in place', () => {
        // The lean path relies on this: `normalize` in @repositories/base maps over rows and
