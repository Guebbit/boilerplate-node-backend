...
describe('userService cart operations', () => {
    it('cartItemSetById adds a new product to an empty cart', async () => {
        ...
        const result = await cartService.cartItemSetById((user._id as Types.ObjectId).toString(), pid, 3);
    ...
    it('cartItemSetById overwrites the quantity when the product is already in the cart', async () => {
        ...
        const firstResult = await cartService.cartItemSetById((user._id as Types.ObjectId).toString(), pid, 2);
        const updatedUser = (firstResult as IResponseSuccess<IUserDocument>).data!;

        const secondResult = await cartService.cartItemSetById((updatedUser._id as Types.ObjectId).toString(), pid, 7);
    ...
    it('cartItemAddById increases the quantity of an existing cart item', async () => {
        ...
        const setResult = await cartService.cartItemSetById((user._id as Types.ObjectId).toString(), pid, 2);
        const userAfterSet = (setResult as IResponseSuccess<IUserDocument>).data!;

        const addResult = await cartService.cartItemAddById((userAfterSet._id as Types.ObjectId).toString(), pid, 3);
    ...
    it('cartItemRemoveById removes the specified product from the cart', async () => {
        ...
        const addResult = await cartService.cartItemSetById((user._id as Types.ObjectId).toString(), pid, 1);
        const userWithItem = (addResult as IResponseSuccess<IUserDocument>).data!;

        const removeResult = await cartService.cartItemRemoveById((userWithItem._id as Types.ObjectId).toString(), pid);
    ...
    it('cartRemove empties the entire cart', async () => {
        ...
        const addResult = await cartService.cartItemSetById((user._id as Types.ObjectId).toString(), pid, 5);
        const userWithCart = (addResult as IResponseSuccess<IUserDocument>).data!;

        const clearResult = await cartService.cartRemove((userWithCart._id as Types.ObjectId).toString());
    ...
    it('cartGet returns populated cart items with product details', async () => {
        ...
        const addResult = await cartService.cartItemSetById((user._id as Types.ObjectId).toString(), pid, 2);
        const userLoaded = (addResult as IResponseSuccess<IUserDocument>).data!;

        const items = await cartService.cartGet((userLoaded._id as Types.ObjectId).toString());
    ...
    it('cartItemSet (by document) is equivalent to cartItemSetById', async () => {
        ...
        const result = await cartService.cartItemSet((user._id as Types.ObjectId).toString(), product, 4);
    ...
    it('cartItemAdd (by document) increases quantity', async () => {
        ...
        const setResult = await cartService.cartItemSetById((user._id as Types.ObjectId).toString(), pid, 1);
        const userLoaded = (setResult as IResponseSuccess<IUserDocument>).data!;

        const addResult = await cartService.cartItemAdd((userLoaded._id as Types.ObjectId).toString(), product, 9);
    ...
    it('cartItemRemove (by document) removes the product', async () => {
        ...
        const addResult = await cartService.cartItemSet((user._id as Types.ObjectId).toString(), product, 1);
        const userLoaded = (addResult as IResponseSuccess<IUserDocument>).data!;

        const removeResult = await cartService.cartItemRemove((userLoaded._id as Types.ObjectId).toString(), product);
...
describe('cartService.orderConfirm', () => {
    it('creates an order from the cart and empties the cart afterwards', async () => {
        ...
        const orderResult = await cartService.orderConfirm((userWithCart._id as Types.ObjectId).toString());
    ...
    it('rejects with 409 when the cart is empty', async () => {
        const user = await createUser();
        const result = await cartService.orderConfirm((user._id as Types.ObjectId).toString());
...
describe('cartService.productRemoveFromCartsById', () => {
    it('removes a product from every user cart that contains it', async () => {
        ...
        const addResult1 = await cartService.cartItemSetById((user1._id as Types.ObjectId).toString(), pid, 1);
        const addResult2 = await cartService.cartItemSetById((user2._id as Types.ObjectId).toString(), pid, 2);
        const id1 = (
