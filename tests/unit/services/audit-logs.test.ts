/**
 * `src/services/audit-logs.ts` — the persistence sink behind `@core/observability/audit`.
 *
...
jest.mock('@repositories/audit-logs', () => ({
    auditLogRepository: {
        create: jest.fn(),
        search: jest.fn()
    }
...
describe('auditLogService.record', () => {
    ...
    it('names the failing action in the warning, so a lost entry is identifiable', async () => {
        ...
        auditLogService.record(makeEntry({ action: AuditAction.AUTH_LOGIN_FAILED }));
        ...
        expect(mockedLogger.warn).toHaveBeenCalledWith(
            expect.objectContaining({ action: AuditAction.AUTH_LOGIN_FAILED })
        );
    });
});
