/**
 * `scripts/eslint/no-persistence-imports` still fires. See `comment-links.test.ts` for why these
 * four rule tests carry one known-bad input each and no case table.
 */
import { RuleTester } from 'eslint';
import { noPersistenceImports } from '../../../../scripts/eslint/no-persistence-imports';
