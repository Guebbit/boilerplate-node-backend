/**
 * `scenarios/seed.ts` — the insert-if-absent policy every scenario module seeder goes through.
 *
...
import { insertIfAbsent } from '@scenarios/seed';
...
it('creates when no document carries the pinned id', async () => {
    ...
    await expect(insertIfAbsent(repository, FIXTURE)).resolves.toBe('created');
...
it('skips when the id already exists — a second boot is a no-op, not a rewrite', async () => {
    ...
    await expect(insertIfAbsent(repository, FIXTURE)).resolves.toBe('skipped');
