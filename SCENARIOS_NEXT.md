...
| 6 D1   | every order a state the app could reach: `status` stored, history `delivered`/`shipped`/`cancelled`, the 10-unit out-of-stock line dropped, `order.ownerPending` holds real stock via `reserveForOrder`          | `b5891c9a`                                             | the docblocks of `orders/factories.ts`, `scenarios/orders.ts`, `scenarios/index.ts`'s `seedShop`                                                                     |
| 6 D2   | every audit row points at state the seed agrees with; `actor_scope` added; the refund row dropped (no payment exists to name)                                                                                    | `283d644e`                                             | `scenarios/audit-logs.ts`'s own docblock and inline comments                                                                                                         |
| 6 D3   | the `es`/`fr` locale overrides key against real paired-frontend strings, not invented ones                                                                                                                       | `20eafe5b`                                             | `scenarios/locales.ts`'s own docblock                                                                                                                                |
| 6 D4   | `POST /products` writes the opening `receive` movement, through a `product.created` domain event (`inventory` already imports `products`, so the reverse direction would cycle)                                  | `51a7ee6d`                                             | `products/events.ts`, `products/service.ts`'s `create()`, `inventory/module.ts`                                                                                      |
| 6 D5   | the demo webhook sink is actually deliverable: an exact-hostname SSRF exemption, dev/test-only, boot refuses it in production; a fixed documented ring secret                                                    | `672e1476`                                             | `ssrf-guard.ts`'s module docblock, `docs/modules/webhooks.md#seeing-it-work`                                                                                         |
| 6 nits | order snapshots freeze `categories`/`tags`/`thumbnailUrl`/`requiresShipping`; sixteen history orders' `createdAt` spread across 2024; `placekitten.com` replaced by a committed local placeholder                | `a9ff45ca` `6542c1b3`                                  | `scenarios/orders.ts`, `products/model.ts` + `users/model.ts`                                                                                                        |
| 6 D7   | the paired frontend's two stale specs fixed: `orders.cy.ts` reads seeded rows instead of provisioning a redundant one; `storefront.cy.ts`'s reorder case asserts the reordered product, not just a nonempty cart | frontend `18f4062f`                                    | those two specs' own comments                                                                                                                                        |
...
- **5 is done.** It came after 4 because it moved the files 4 edited; 6 (now done and deleted) and
  7 are unblocked.
