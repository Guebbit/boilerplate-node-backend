#!/usr/bin/env tsx
/**
 * The baseline index migration, generated from the schemas that declare the indexes.
 *
 * Hand-maintaining the table meant two edits per index — the schema, then the migration — with
 * only a test to catch the forgotten second one. Reading the schemas instead leaves one author,
 * the same shape the contract bundler gives `openapi.yaml`. Run `--check` to fail when the
 * committed file has diverged; that is what runs in `complete`.
 *
...
const MIGRATION_PATH = path.join(ROOT, 'db', 'migrations', MIGRATION_FILE);
...
const main = async () => {
    ...
    const source = await format(renderMigration(entries), {
        ...prettierConfig,
        filepath: MIGRATION_PATH
    });

    const committed = fs.existsSync(MIGRATION_PATH)
        ? fs.readFileSync(MIGRATION_PATH, 'utf8')
        : null;

    if (checkOnly) {
        if (committed === source) {
            console.info(`[gen:migration] db/migrations/${MIGRATION_FILE} is up to date.`);
            process.exit(0);
        }

        console.error(
            `[gen:migration] db/migrations/${MIGRATION_FILE} is stale — run \`npm run gen:migration\`.`
        );
        process.exit(1);
    }

    fs.writeFileSync(MIGRATION_PATH, source);
    console.info(
        `[gen:migration] wrote db/migrations/${MIGRATION_FILE} — ${entries.length} indexes.`
    );
    process.exit(0);
};
