...
| Cross-purpose key use    | one key for signing and encryption, or for access and refresh | Five separate secrets — `NODE_TOKEN_ACCESS`, `NODE_TOKEN_REFRESH`, `NODE_TOTP_ENCRYPTION_KEY`, `NODE_WEBHOOK_SECRET_ENCRYPTION_KEY`, `NODE_PII_ENCRYPTION_KEY` — each required independently at boot. |
...
| PII at rest in plaintext     | a database dump exposes names, addresses, phone numbers, not just credentials | An address-book entry's fullName/street/city/zip/country/phone, and a user's own phone, are AES-256-GCM under a versioned key — same primitive as the row above, a different key (`NODE_PII_ENCRYPTION_KEY`) — `infrastructure/security/pii-encryption.ts`, `addresses/repository.ts`, `users/model.ts#toUser` |
