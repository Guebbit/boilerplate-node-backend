...
| ------------------------------- | ------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Missing or optional TLS         | HTTP endpoints, no redirect, cookies sent in the clear  | Not this process's layer — `docker-compose.production.yml` binds the API to loopback, so a TLS-terminating proxy is structurally required rather than merely advised. |
| SSL stripping / downgrade       | no HSTS, no preload; an on-path attacker rewrites links | `helmet()` sets `Strict-Transport-Security` on every response this app sends — `app/security.ts`. Preload registration is the domain owner's.                         |
| Weak TLS configuration          | TLS 1.0/1.1, RC4, export ciphers, no forward secrecy    | The terminating proxy's. See [Infrastructure](infrastructure.md#the-network-and-the-platform).                                                                        |
