...
| Attack                       | How it works                                   | This boilerplate                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
| ---------------------------- | ---------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Email bombing / resend abuse | unlimited resend endpoints                     | Two mechanisms, because they answer different things. `credentialLimiters` on `/reset`, `/verify-request` and the 2FA send route bound the attempts, and `mfaSendLimiter` bounds outbound CODES separately from guesses, since sending and guessing are different budgets — `infrastructure/http/middlewares/rate-limit.ts`. But those limiters carry `skipSuccessfulRequests`: only a FAILURE spends the budget, which is right for a login guess and wrong for a button whose success publishes the mail. So each re-send also holds a per-account cooldown, returned as `resendAfter` so a client counts down rather than inventing its own — `account/services/verification.ts`, `account/services/two-factor.ts` |
...
| Attack                     | How it works                                                    | This boilerplate                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| -------------------------- | --------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Unverified email at signup | the account is bound to an address the user does not own        | Enforced through the permission model: a fresh signup holds `unverified`, which grants no `cart.checkout` — it cannot place an order or start a payment, so an unproven address never triggers order mail. Promoted to `customer` once the mailbox is proven — by `POST /account/verify-confirm`, or by spending a password-reset token, which was delivered to that same address and nowhere else — `shared/authorization-roles.yaml`, `kernel/middlewares/authorizations.ts#requirePermission`, `account/services/profile.ts#passwordResetChange`. |
| Pre-account-takeover       | the attacker registers the victim's address, who later links in | Closed on both sides: the squatted account cannot spend, AND the OAuth link path now demands a verified address of the account it links onto, not only of the provider — see [Federated login](authentication.md#federated-login).                                                                                                                                                                                                                                                                                                                   |
