...
| Plaintext wire traffic          | a keyFile authenticates replica-set members, it does not encrypt what they send each other or the app | `database` runs `--tlsMode requireTLS` under a self-signed CA `mongo-entrypoint.sh` mints; `NODE_DB_URI` and every in-container `mongosh`/`mongodump` call carry `tlsCAFile` to verify it — `docker-compose.production.yml`, `docker/mongo-entrypoint.sh`                |
...
| Unencrypted at rest             | a disk image or backup is readable as-is                                                              | Disk-level is the host's, see [Encryption at rest](../../tools/deployment-hardening.md#encryption-at-rest); field-level, see [Secrets at rest](crypto-and-secrets.md#secrets-at-rest). Backups are already encrypted client-side, see [Backups](../../tools/backups.md). |
...
| Orphaned / residual data        | soft-deleted rows still readable; missing cascades            | Soft-deleted rows are filtered by each module's own compiled scope (`orderService.callerScope` and siblings), merged into the read the same way the owner clause above is, not by each caller remembering to — `infrastructure/persistence/create-repository.ts` |
