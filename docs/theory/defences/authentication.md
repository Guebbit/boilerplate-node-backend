...
| -------------------------------- | --------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Brute force                      | no lockout, no rate limit, no MFA                                     | `credentialLimiters` — two independent budgets, per account and per address, spent only by FAILURES, so an honest user never pays — `account/rate-limits.ts#credentialLimiters`                                                                                                                                                                                                                                                                                                                                         |
...
| Default / hard-coded credentials | `admin/admin`, demo users, credentials in the repo                    | The seed interlock refuses production outright, and refuses everywhere else too when any seed account is still logging in with its committed, public fallback password — `scenarios/apply.ts`, `scenarios/accounts.ts`                                                                                                                                                                                                                                                                                                  |
| Weak password policy             | short, common or breached passwords accepted                          | `PasswordNew` in the contract bars the shape; a breached password is caught on top of that, on every SET path (signup, reset, change, admin-issued) — a committed list of ~20k previously-breached-yet-composition-valid passwords, then the HIBP k-anonymity range API for the tail, both failing open — `infrastructure/security/breached-passwords`. `POST /account/password/check` is an advisory-only preview of both, and the paired frontend adds a `zxcvbn` strength meter as a third, non-blocking suggestion. |
...
| Attack                                | How it works                                            | This boilerplate                                                                                                                                                                                                                                                             |
| ------------------------------------- | ------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| OTP / 2FA bypass — attempt cap        | no rate limit on code guesses                           | A dedicated limiter bounds guesses against ONE still-live login challenge, independent of the account and address budgets — `account/rate-limits.ts#mfaChallengeLimiter`                                                                                                     |
...
| OAuth — missing PKCE                     | an intercepted authorization code is redeemable                                                                      | S256 PKCE ships beside `state`, not instead of it — the client secret already covers a confidential client, this covers the code leaking before the secret does — `account/oauth/state.ts`, `account/oauth/providers/`                                                                                                                                                                                                                                                                         |
...
| OAuth — 2FA bypass via a linked provider | an armed second factor guarded only the password path; signing in with the linked provider minted a session directly | The callback checks `twoFactorEnabledAt` exactly like `postLogin`'s 2FA branch and issues the same login challenge instead of a session. The challenge crosses the redirect in a short-lived httpOnly cookie, never the URL — `account/controllers/get-oauth-callback.ts`, `account/oauth/mfa-redirect.ts`                                                                                                                                                                                     |
...
| Pre-account-takeover                     | the attacker registers the victim's address before they do                                                           | Two halves, both closed. Signup is bounded — an unproven address holds `unverified` and no `cart.checkout`. And the LINK now demands verification of BOTH sides: `loginOrCreateFromOAuth` refuses to attach a provider identity to an account that never proved the address itself, so the victim's own Google sign-in cannot drop them into a squatter's account. Their way in is the password reset, which proves the same mailbox — `account/services/oauth.ts#OAuthAccountUnverifiedError` |
