...
| Stale connection authorization    | rights revoked, but the socket lives on                            | Re-checked every 30 seconds; the stream closes on the first failed check. Same verdict as connecting — `kernel/middlewares/authorizations.ts#stillHoldsKeyViaCookie`, called from `infrastructure/observability/stream.ts#streamObservabilityMetrics` |
