...
| Attack                            | How it works                                                       | This boilerplate                                                                                                                                                                                                                                        |
| --------------------------------- | ------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Broadcast leakage                 | room or topic scoping errors fan a message too widely              | The stream carries PROCESS metrics only: request volumes, error rates, latency, uptime, heap. No user data fans out through it — `modules/observability/services/stream.ts`                                                                             |
...
| Stale connection authorization    | rights revoked, but the socket lives on                            | Re-checked every 30 seconds; the stream closes on the first failed check. Same verdict as connecting — `kernel/middlewares/authorizations.ts#stillHoldsKeyViaCookie`, called from `modules/observability/services/stream.ts#streamObservabilityMetrics` |
