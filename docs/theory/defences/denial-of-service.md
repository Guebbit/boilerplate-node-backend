...
| Storage exhaustion           | unlimited uploads or log flooding fill the disk        | Per-file byte ceiling plus the dedicated `uploadLimiter` budget — `rate-limit.ts#uploadLimiter`. Image LIFETIME follows its document — a replaced avatar/product image deletes the old one, a hard delete deletes the row's own — which bounds one account to its one current image rather than every image it ever uploaded; see [Files](files-and-uploads.md#stored-file-lifetime). A global quota across all uploads is 🚧 coming soon. |
...
| Cache stampede               | expiry makes every worker rebuild the same key at once | Refresh-ahead in the app plus `stale-while-revalidate` at the edge — one caller rebuilds, the rest read the stale value; see [HTTP and caches](http-and-caches.md#caches).                                                                                                                                                                                                                                                                 |
...
| Attack                        | How it works                                         | This boilerplate                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| ----------------------------- | ---------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Email / SMS bombing           | unlimited resend endpoints; SMS pumping (toll fraud) | `credentialLimiters` on `/reset`, `/verify-request` and the 2FA send route; `mfaSendLimiter` bounds outbound codes separately from guesses — `account/rate-limits.ts`. A limiter alone is not enough where a SUCCESS is the expensive call, since `skipSuccessfulRequests` means only failures spend the budget — so the two re-send endpoints carry their own cooldown; see [Email](email.md#how-much-mail). SMS pumping has no surface: there is no SMS factor. |
