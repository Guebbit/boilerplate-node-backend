...
| Erasure                | 17      | `DELETE /account` (self, emailed confirmation) or admin `DELETE /users/:id?hardDelete=true` | the subject, or an admin              | `orders`/`payments` are anonymised on a delay (`NODE_ORDER_PII_RETENTION_DAYS`), not deleted — invoices are kept for tax law. A **soft** delete (`hardDelete` unset) is reversible and does **not** discharge an erasure request — only `hardDelete=true` does. **Refused with 409** for a shop's last `admin`, self-service and admin alike — the request is honoured once somebody else has been given that role |
