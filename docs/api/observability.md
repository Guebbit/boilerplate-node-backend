...
installation, not about one shop, so the key they check is `platform.observability.any.read` and a
...
- **`platform.observability.any.read` via bearer token**: `/observability/health`, `/observability/metrics/overview`, `/observability/audit`
- **`platform.observability.any.read` via refresh cookie**: `/observability/events` (`requirePermissionViaCookie`)
...
| --- | --- | --- | --- |
| `GET /observability/events` | `platform.observability.any.read`, by cookie | SSE stream: live metrics snapshot every 5 s | [Frontend Observability](../tools/frontend-observability.md) |
| `GET /observability/metrics` | scraper | Raw Prometheus exposition (text/plain) | [Prometheus](../tools/prometheus.md) scrape target |
| `GET /observability/health` | `platform.observability.any.read` | Full health snapshot: DB status, memory, CPU, integrations, uptime | [Grafana](../tools/grafana.md) health panels |
| `GET /observability/metrics/overview` | `platform.observability.any.read` | Curated KPI JSON: HTTP totals, error rate, latency p50/p95, auth & business counters | [Prometheus](../tools/prometheus.md) / [Grafana](../tools/grafana.md) KPI panels |
| `GET /observability/audit` | `platform.observability.any.read` | Recent audit events from the persisted audit trail, newest first | [Loki](../tools/loki.md) log search |
...
| `action` | string | Filter by action (e.g. `order.created`) |
