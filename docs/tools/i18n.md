...
call site — `scripts/eslint/no-hardcoded-user-text.ts` enforces it. This page is how the dictionary
...
`@infrastructure/i18n` is a directory with a barrel, not a file. Three change drivers, three files:
...
Turning an `Accept-Language` header into one supported locale is not in this directory at all — it
is `http/middlewares/locale.ts`'s own `negotiateLocale`, a thin wrapper around Express's
`request.acceptsLanguages` (see [Request Flow](../theory/request-flow.md)), not a hand-rolled
parser this directory owns.
...
`localeCandidatesFor(locale)` (`src/infrastructure/i18n/catalog.ts`) turns one negotiated
...
(`resolveTranslations`/`applyTranslations` in `src/kernel/translation.ts`):
