...
| **Readiness**   | `modules/observability/dependency-health.ts` (no I/O) + `job-health.ts` (one `leases` query) — the module's own, reading each adapter's connection state directly | —                                                                               | `GET /observability/health`                                     |
| **Live stream** | `modules/observability/stream.ts` — SSE, 5 s updates, 15 s heartbeat — the module's own                                                                           | —                                                                               | `GET /observability/events`, shape pinned by `asyncapi.yaml`    |
