...
All of this is wired in `src/core/bootstrap/otel-sdk.ts`. `startTracing()` is called at the very top of `src/app.ts`, before Express and any instrumented libraries are imported — this is required for auto-instrumentation to work.
