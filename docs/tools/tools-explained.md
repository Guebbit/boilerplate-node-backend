...
**In this repo.** `GET /observability/events` streams live process metrics — an initial snapshot, then updates every 5 s and a heartbeat every 15 s. It is an ordinary Express route (`src/modules/observability/stream.ts`), so all the usual middleware applies. Event names and payload shapes come from `asyncapi.yaml`.
