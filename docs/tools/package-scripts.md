...
| `npm run complete:check`   | before committing — the same gate pre-commit runs                                           |
...
| `host`               | prefix wrapper: `npm run host -- dev` runs any script against `localhost` datastores       | [Runtime](./runtime.md)                   |
...
| `test:load`              | autocannon against `GET /products` — the cached read path                                                                           | [Load Testing](./load-testing.md)              |
| `test:load:search`       | autocannon against `POST /products/search` — the uncached, database-backed path                                                     | [Load Testing](./load-testing.md)              |
...
redirects the **hostname** to `localhost` via `cross-env`, by blanking `NODE_DB_URI` /
`NODE_REDIS_URL` so the resolvers fall through to `NODE_MONGODB_HOST` / `NODE_REDIS_HOST`, leaving
the port and the database name to `.env`. The shipped `.env` uses compose hostnames, so the bare
