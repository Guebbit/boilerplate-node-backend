...
```bash
npm run docker:restart     # or: npm run dev
npm run load:test          # GET /products      — cached read path
npm run load:test:search   # POST /products/search — uncached, database-backed
```
...
`load:test` hits `GET /products`, which is cached (`setCache(3600, { tags: ['products'] })` in
...
`load:test:search` hits `POST /products/search`, which is **not** cached and runs a regex query
