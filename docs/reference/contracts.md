...
hand-edited. `npm run check:contracts-bundle`, `check:asyncapi-types` and `check:spec-identity`
each fail when a committed copy disagrees with a fresh run.
:::
...
| `shared/contracts/analytics.frontend.ts` | The analytics event names only a _client_ can emit — an app start, a logout, a checkout that never arrived. The mirror of the workers document: names no module owns because this service never observes the moment they describe. | [Frontend Observability](../tools/frontend-observability.md) · [Product Analytics](../tools/analytics.md) |
