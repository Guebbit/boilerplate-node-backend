...
| `scripts/docs/generate-role-matrix.ts`  | Writes the two role tables in `docs/demo-ecommerce/index.md` — declared keys and what `holdsKey` actually answers — read off the permission model itself, not typed out by hand. `npm run docs:roles`; its check mode is the gate. | [Authorization](../theory/authorization.md)   |
| `scripts/docs/generate-attack-index.ts` | Writes the A-Z lookup in `docs/theory/defences/index.md`, harvested from the defence pages' own attack tables so a renamed heading cannot leave a dead anchor behind — `npm run docs:attacks`.                                     | [Web Attacks & Defences](../theory/defences/) |
