...
| ------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------- |
| `scripts/bundle-contracts.ts`               | The CLI — `npm run contracts:bundle`, and a check mode for the CI gate that fails when a committed bundle disagrees with a fresh run.                                                         | [Contract Ownership & Fragmentation](../api/contract-fragmentation.md) |
| `scripts/contracts/index.ts`                | The catalogue of every document this repo produces from sources it owns, and which of them are guarded.                                                                                       | [Contracts](./contracts.md)                                            |
| `scripts/contracts/fragments.ts`            | The fragment bundler underneath all of them: takes a root document plus one fragment per module and merges them in a fixed order, so a new domain does not mean editing a list in five files. | [Contract Ownership & Fragmentation](../api/contract-fragmentation.md) |
| `scripts/contracts/openapi.ts`              | Compiles `openapi.yaml` from the root preamble and each module's standalone OpenAPI document.                                                                                                 | [OpenAPI Workflow](../api/openapi-workflow.md)                         |
| `scripts/contracts/asyncapi.ts`             | Compiles both async bundles — the full `asyncapi.yaml` and the `asyncapi.public.yaml` the frontend receives — from one set of sources.                                                        | [AsyncAPI Workflow](../api/asyncapi-workflow.md)                       |
| `scripts/contracts/analytics-events.ts`     | Builds the analytics event-name contract: the module-owned names plus the client-only ones, published to the paired frontend.                                                                 | [Product Analytics](../tools/analytics.md)                             |
| `scripts/contracts/generate-collections.ts` | Writes the four client collections from `openapi.yaml` — one request at a time, with auth, bodies and example responses.                                                                      | [Contracts](./contracts.md)                                            |
| `scripts/gen-asyncapi-types.ts`             | Generates `src/types/asyncapi.generated.ts` from `asyncapi.yaml`. Its check mode is the gate. Byte-identical to the frontend's copy.                                                          | [AsyncAPI Workflow](../api/asyncapi-workflow.md)                       |
| `scripts/regenerate.ts`                     | Runs every generator in the right order — `npm run regenerate`. The one command to reach for after editing anything a generator reads.                                                        | [Regenerating After a Change](../api/regenerating.md)                  |
...
| File                             | What it is                                                                                                                                                                                                            | Read next                                        |
| -------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------ |
| `scripts/frontend-path.ts`       | Where the paired frontend is expected to be — a sibling checkout by default, overridable by environment. Mirrored on the other side.                                                                                  | [Pairing & Ports](../tools/pairing-and-ports.md) |
...
| `scripts/sync-frontend.ts`       | Copies every backend-owned shared file into the paired frontend — `npm run sync:frontend`. The write side of what the identity check only verifies.                                                                   | [Pairing & Ports](../tools/pairing-and-ports.md) |
...
| File                                 | What it is                                                                                                                   | Read next                                        |
| ------------------------------------ | ---------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------ |
| `scripts/mutation.ts`                | Runs Stryker — `npm run test:mutation`. A wrapper rather than a bare invocation, for the three jobs a JSON config cannot do. | [Mutation Testing](../tools/mutation-testing.md) |
...
| File                        | What it is                                                                                                                                                    | Read next                                          |
| --------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------- |
| `scripts/test-report.ts`    | Turns a runner's JSON report into which **module** a failure belongs to and where the time went — `npm run test:report`.                                      | [Testing (overview)](../tools/testing-and-docs.md) |
| `scripts/heap-report.ts`    | Summarises a V8 heap snapshot by object kind. The first tool to reach for when a worker dies of memory rather than of a failing assertion.                    | [Mutation Testing](../tools/mutation-testing.md)   |
| `scripts/heap-retainers.ts` | Answers "who is holding these?" for one kind of object — the question the summary cannot, because it aggregates nodes and never reads the edges between them. | [Mutation Testing](../tools/mutation-testing.md)   |
