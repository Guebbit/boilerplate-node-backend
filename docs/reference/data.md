...
| ------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------- |
| `scenarios/apply.ts`                       | The seeder that `npm run scenario:apply` runs. Boots the application in-process and hands `scenarios/index.ts`'s `buildScenario` a loopback listener: the catalogue is seeded from the module table, and the order book is then produced by driving the real endpoints. A module absent from the table seeds nothing. Refuses a database that already holds anything unless `--reset` empties it first.                                                                                                 | [Modules](./src-modules.md) · [Demo profile](../tools/demo-profile.md) |
