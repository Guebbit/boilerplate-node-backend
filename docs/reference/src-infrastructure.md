...
| `src/infrastructure/http/middlewares/rate-limit.ts`     | Rate limiting and the credential check on the metrics endpoint, plus the default window and per-IP budget used when the rate-limit variables are unset.                                       | [Security](../tools/security.md)                                                           |
...
| `src/infrastructure/persistence/seed.ts`              | The upsert primitive every module's seed file goes through. Knows a repository shape and a factory's output, and nothing about any domain.                                 | [Data](./data.md)                                                                  |
