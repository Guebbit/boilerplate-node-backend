...
| File                                              | What it is                                                                                                                                                                                              | Read next                                                |
| ------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------- |
| `src/infrastructure/adapters/cache.ts`            | The Redis adapter. Every function **fails open** — an unreachable Redis means a cache miss, never a failed request.                                                                                     | [Redis Cache](../tools/redis-cache.md)                   |
...
| `src/infrastructure/http/middlewares/cache.ts`          | Declares which query parameters an endpoint's answer actually depends on, which is what makes its cache key correct rather than merely unique.                                           | [Redis Cache](../tools/redis-cache.md)                                                     |
...
| `src/infrastructure/persistence/factory.ts`         | The part every module's fixture builder would otherwise repeat — an id, a pair of timestamps, and the typing of the overrides bag.                                         | [Unit Testing](../tools/unit-testing.md)                                           |
...
| File                         | What it is                                                                                                                                                              | Read next                                                                            |
| ---------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------ |
| `src/infrastructure/i18n.ts` | Request-scoped i18n: loads the app and module bundles, and hands each request a translator fixed to its negotiated locale so nothing reads a global "current language". | [Internationalisation](../tools/i18n.md) · [Request Flow](../theory/request-flow.md) |
