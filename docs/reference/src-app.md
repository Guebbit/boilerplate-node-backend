...
| `src/app/required-config.ts` | Boot-time checks for what belongs to neither a module's manifest nor the kernel — `NODE_URL`/`NODE_CORS_ORIGIN`, the SMTP companions, and the analytics/mail-transport/log-personal-fields provider selectors — handed to `registerModules` as its `NonModuleChecks` argument. | [Modules](../theory/modules.md)                    |
...
| `src/app/demo.ts`            | The demo profile's control surface — two routes, mounted only when `enableDemoProfile()` was called. Consumed by the paired frontend's e2e suite, and fenced off by the `eslint-plugin-boundaries` element graph in `eslint.config.ts`.                                        | [Demo profile](../tools/demo-profile.md)           |
...
| File                                       | What it is                                                                                                                                                                                                                                                                                          | Read next                                                                                        |
| ------------------------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------ |
| `src/kernel/registry.ts`                   | The thesis of the repository: a module is a typed value, not a folder convention. Defines `AppModule` — name, `basePath`, router, dependencies, locales, scenario guarantees, event subscriptions — and validates the set at boot.                                                                  | [Modules](../theory/modules.md) · [Strategic DDD](../theory/strategic-ddd.md)                    |
...
| `src/kernel/translation.ts`                | The same seam, for user-authored content: the kernel declares the `TranslationPort`, `modules/locales` answers it. Lets `products`/`orders` resolve a translated field without either importing `locales`, and without `locales`' own DTOs leaking into infrastructure.                             | [Internationalisation](../tools/i18n.md)                                                         |
...
| `src/kernel/permissions.ts`                | The declared permission keys and preset roles, parsed and validated off `shared/authorization-keys.yaml`/`shared/authorization-roles.yaml` — the same two files the PHP twin reads byte-for-byte.                                                                                                   | [Authorization](../theory/authorization.md)                                                      |
| `src/kernel/ability.ts`                    | A resolved caller turned into the CASL `Ability` every authorization question is asked of, built per request from the keys `permissions.ts` says the caller's role holds.                                                                                                                           | [Authorization](../theory/authorization.md)                                                      |
| `src/kernel/access/query.ts`               | The other half of the route guard: once a caller is through, which ROWS may they read. `accessibleFilter` compiles the caller's own rules into the filter a collection is read with — a role that reads everything narrows nothing, and a caller with no rule matches nothing rather than widening. | [Authorization](../theory/authorization.md)                                                      |
| `src/kernel/access/tenant.ts`              | The one shop's pinned `_id`, in its own file rather than `@modules/access`'s to avoid a circular import — `permissions.ts` needs the constant too.                                                                                                                                                  | [Authorization](../theory/authorization.md)                                                      |
...
| `src/kernel/required-config.ts`            | The boot-time gate's mechanism: collects every module's own `requiredConfig` plus the app tier's `NonModuleChecks` (`src/app/required-config.ts`) into one pass, so a misconfigured deployment names every mistake at once.                                                                         | [Ops & Assets](./ops.md)                                                                         |

## `src/types/`

| File                              | What it is                                                                                                                                                                                                                                                                   | Read next                                             |
| --------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------- |
| `src/types/index.ts`              | The single import path for contract types: re-exports the Orval models from `api/models` and the generated AsyncAPI types, plus the auth DTO. Import from `@types`, never from the generated files directly, and never restate a generated shape by hand under its own name. | [Regenerating After a Change](../api/regenerating.md) |
