...
| `src/app/telemetry.ts`       | The Prometheus HTTP metrics middleware. Mounted after the request context so route labels come from a request the stack has already matched, rather than from a raw URL with ids in it.                                                | [Prometheus](../tools/prometheus.md)               |
