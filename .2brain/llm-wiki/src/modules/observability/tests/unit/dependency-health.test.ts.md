...
- **`describe('dependencyHealth')`** — asserts the mongoose `readyState →` word mapping (1→`ready`, 2→`connecting`, 0→`unavailable`, 3→`unavailable`, 99→`unavailable`) and that the service *asks* each adapter for its state rather than probing it directly.
...
- `disabled` is a *word* (the dependency is intentionally absent) but not a *status*; `connecting` is a distinct word from `unavailable` yet both fold to `degraded`. The test encodes both distinctions.
