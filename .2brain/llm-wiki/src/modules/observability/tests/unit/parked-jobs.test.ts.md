...
- **`describe('queueHealth')`** – Two cases:
  - *reports every queue parkedCounts answers with, unchanged* – asserts the array of `{ name, parked }` objects arrives verbatim (no Date-to-string mapping, no reordering).
  - *answers empty when parkedCounts reaches no queue at all* – asserts an empty array is passed through.
