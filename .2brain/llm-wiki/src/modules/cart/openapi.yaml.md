...
- **422 vs 404 convention.** `422` means the id (path or body) is *malformed* (e.g., not a valid ObjectId). `404` means the id is well-formed but matches no product or cart line. This split is called out in inline comments on the affected operations.
