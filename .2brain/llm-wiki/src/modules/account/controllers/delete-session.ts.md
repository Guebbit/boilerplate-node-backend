...
- Revoking the caller's _current_ session is explicitly allowed and behaves like `POST /account/logout` except it does **not** clear cookies (this endpoint is meant to target a different client's session).
