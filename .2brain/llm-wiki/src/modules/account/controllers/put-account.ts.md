...
- **No `= ''` default on `imageUrl`.** `updateProfile` treats an *absent* `imageUrl` as "not sent" and preserves the stored value; an empty string would *clear* it. Deliberate divergence from the create-path controllers.
