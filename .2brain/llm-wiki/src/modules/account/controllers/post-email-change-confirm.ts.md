...
- **Derived status codes:** The `catch` block does *not* hardcode 500. If the new address was claimed by another account between token issuance and confirmation, the DB unique-index rejection surfaces as a `409` via `catchAs`.
