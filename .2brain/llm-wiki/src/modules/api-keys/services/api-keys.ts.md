...
## Key elements
- **`isMintable(key, caller)`** (internal) — Returns `true` only when the permission key is *declared* in the permission registry, scoped to `tenant`, and *held* by the caller via `holdsKey`.
...
- **Permissions are checked twice.** Subset validation happens at mint time here *and* on every subsequent request via `module.ts`'s `CredentialResolver`, so a demoted minter cannot retain over-privileged keys.
