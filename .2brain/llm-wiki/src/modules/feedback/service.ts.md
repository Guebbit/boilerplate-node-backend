...
- **`updateStatus(feedback, payload)`** — Patches `status` and/or `adminNotes` on an already-loaded document and saves. Stamps `respondedAt` only on the *first* transition to `resolved`; re-resolving does not move the timestamp.
...
- **Operator email locale is pinned.** `contactRequestEmail` is called with `getDefaultLocale()` explicitly, *not* with the caller's `CallerContext`. The submitter's `subject`/`message` pass through verbatim.
