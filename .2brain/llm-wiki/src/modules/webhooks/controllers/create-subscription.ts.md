...
- **URL SSRF re-validation is deferred.** The `url` field is checked for `https://` at request time via the Zod schema pattern, but the actual SSRF guard (resolving DNS and rejecting private/link-local IPs) runs on _every delivery_, not at subscription time, because DNS can change post-creation. See `@infrastructure/adapters/ssrf-guard.ts`.
