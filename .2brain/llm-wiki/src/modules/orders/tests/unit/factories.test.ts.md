...
- **`describe('makeOrder — identity and defaults')`** — asserts: `_id`/`userId` are real `Types.ObjectId`; `userId` is unique per call; `items` defaults to `[]`; optional fields (`shippingMethod`, `shippingAddress`, `notes`, `deletedAt`) are *absent* (not set to `null`); `shippingCost: 0` is preserved; ISO-string `deletedAt` becomes a `Date`.
...
- The `@ts-expect-error` test is the *type-level* half of the stock-counter exclusion guarantee; the write-time half (that a live-stock field on the actual product document is still stripped) is covered in `orders/tests/integration/schema-contract.test.ts`.
