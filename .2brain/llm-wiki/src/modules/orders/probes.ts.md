...
## Relationships
- **`scripts/contracts/client-collections-bundle.ts`** — The file that consumes this export. Per the module doc comment, that script defines what a probe is for, *emits* these probes into the generated collection, and resolves the `{{seedOrderId}}` / `{{seedDeletedOrderId}}` seed tokens used in probe paths.
