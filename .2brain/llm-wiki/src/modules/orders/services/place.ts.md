...
- **`PlaceOrderShipping`** — caller-resolved address, a `method` whose `priceFor` receives the *frozen* lines (not a precomputed total), and optional `holdMinutes`.
...
- The `ObjectId` is generated *before* `orderRepository.create` so the bank-transfer reference names the same `_id` the row will have. A retried call produces a different id and therefore a different reference — this is what prevents duplicate references for the same logical order.
