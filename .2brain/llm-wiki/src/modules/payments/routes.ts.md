...
- **Middleware order on `/:id/confirm` is deliberate**: velocity limiters and the challenge gate sit *before* `idempotencyKey`, mirroring where `credentialLimiters`/`loginChallengeGate` sit on `POST /account/login`. Reordering changes what gets rate-limited vs. what gets deduplicated.
