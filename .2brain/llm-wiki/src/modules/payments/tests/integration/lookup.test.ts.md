...
- **`transferOrder()`** (local helper) — creates a user, product, and a pending `bank_transfer` order whose `_id` is pinned *before* insertion so `buildReference(id)` names the exact row that is about to be written.
...
- The settle test imports `recordOfflinePayment` from the *existing* offline service — the test's job is to confirm no new settlement code was introduced between lookup and settle, not to test the offline service's own logic (that lives in `service.test.ts`).
