...
- **`subdomain: supporting`** — classifies wishlist under the _supporting_ domain, distinguishing it from core commerce modules (e.g. `orders`, `cart`).
...
- **`src/modules/wishlist/module.ts`** — the TypeScript implementation this YAML file declares. The manifest defines _what_ the module needs; `module.ts` defines _how_ it behaves.
...
- Comments after each dependency are the only documentation of _why_ the dependency exists; removing them loses context for future maintainers.
