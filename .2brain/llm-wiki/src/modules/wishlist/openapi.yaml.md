...
- **POST /wishlist/{productId}/move-to-cart** (`moveWishlistItemToCart`) — Transfers a saved line into the cart (qty 1, or increment). Cart is written _before_ the wishlist line is removed, so a mid-operation failure leaves the product SAVED rather than lost. 422 carries `CART_QUANTITY_LIMIT` if the existing cart line is already at 999.
...
- The 404 on `move-to-cart` is semantically the _cart's_ answer (product no longer publicly visible), not the wishlist's. A saved line can outlive a catalogue entry; the 404 fires at the cart's visibility check.
