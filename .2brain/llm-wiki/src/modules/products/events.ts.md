...
- **`declare module '@kernel/events'` block** — Augments `DomainEventMap` with three payload types:
  - `'product.deleted'` — `{ productId: string; hardDelete: boolean }`. Emitted and awaited *before* the write; `hardDelete` tells listeners whether to drop inventory counters.
...
- **`product.deactivated` ≠ out-of-stock.** It means "gone for a long time"; `onHand: 0` with `active: true` is merely out of stock and does *not* fire this event.
