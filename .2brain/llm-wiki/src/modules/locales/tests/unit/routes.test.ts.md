...
- **`describe("…caching")`** – verifies public reads use `setCache(3600…)` with `tags: ['locales']` and `browserRevalidate: true`; that the editing/translation routes are uncached; that remaining admin writes call `invalidateCache([locales])`; and that translation routes do *not* invalidate that tag (they clear their own registry-declared tag in-service).
...
- `GET /` is the only public route expected to include `getAuth` in its chain; it is *not* expected to include an identity guard. Confusing the two in a refactor will be caught.
- The `TRANSLATIONS` routes are excluded from the "invalidates `locales` tag" assertion *and* from the "uncached" assertion in the sense that they simply must not call `invalidateCache([locales])`; they are, however, expected to be uncached (no `setCache` in their chain).
