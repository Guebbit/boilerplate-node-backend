...
- **Two host env vars, one server.** `NODE_UMAMI_INGEST_HOST` is the address reachable *from the API container* (e.g. `umami:3000` in Compose); `NODE_UMAMI_HOST` is the *public* origin the browser's tracking script loads from. The ingest host takes precedence.
