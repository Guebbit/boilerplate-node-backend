...
- **`sharedFiles(role, suffix?)`** — builds a file→contents map from `SHARED_FILES` keyed by the pair's *index* (not by path), because the two repos may name the same file differently.
...
- The conditional sibling test (present when the sibling checkout is detected) is the only assertion that catches a hand-introduced fork; it reports *skipped* (not passing) when the sibling is absent.
