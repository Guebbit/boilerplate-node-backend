...
- **`src/infrastructure/i18n/context.ts`** — Source of `runWithLocale`, imported via the barrel. The test uses it to create a deliberately _wrong_ ambient locale and proves the worker/producer ignore it.
...
- `payload.data.locale` is asserted to be a **plain string** (`'it'`) used only as the `<html lang>` attribute in the template — it is _not_ a key the worker looks up. The test explicitly documents this to prevent a future developer from "fixing" the worker to re-resolve it.
