...
- **`describe('teardown')`** – Covers `close` being called after success and after failures at each stage (`newPage`, `setContent`, `pdf`). Also documents that a `close` rejection *replaces* the original render error.
...
- The "close failure replaces render failure" test documents a deliberate `finally`-block behavior: if both the render and the teardown reject, the caller sees the *teardown* error. This is asserted, not treated as a bug.
