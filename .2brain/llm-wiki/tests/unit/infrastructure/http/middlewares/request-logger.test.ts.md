...
- The test does **not** exercise the `info`/`warn`/`error` logger methods directly; it asserts the *first argument* passed to `logger.log` (the level string), meaning the middleware funnels all levels through a single `log(level, …)` call.
