...
- **Cleanup discipline:** both `afterEach` hooks restore env vars _and_ call `jest.resetModules()`. Omitting the reset would let a later test file (or a re-run within the same worker) pick up a mutated module cache.
