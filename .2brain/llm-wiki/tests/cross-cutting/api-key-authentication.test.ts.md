...
- **Guard-split test reads source, not requests.** It is a file-level invariant (no module mounts both guards). It uses `codeOf` to strip comments because modules like `orders` and `api-keys` document *why* they deliberately omit a guard in a comment that names the guard; a raw-text match would flag those as violations.
