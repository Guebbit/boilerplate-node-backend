...
- **Deliberate asymmetry:** a bodyless login returns 422, a wrong-password login returns 401. The file explicitly does _not_ assert these are equal—`accountService.login` parses `LoginBody` itself, and the split (shape vs. credential) is intended.
