...
- **`SEED_PRODUCT_IDS`** – `as const` object mapping descriptive names (`dogFoodStandard`, `heaterSoftDeleted`, `scratchPostOutOfStock`, `dogBedPremium`, `bundleInactive`, `barebones`) to their MongoDB hex ids. Each name encodes the product _and_ the edge-case branch it exists to exercise.
...
- **Pinned vs. dynamic.** Only rows that exist _before_ any process runs live here. Orders, payments, and shipments are produced by driving `scenarios/flows/`, so their ids are recorded by the runner and are **not** valid as literals in this file.
