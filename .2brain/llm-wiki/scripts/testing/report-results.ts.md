...
- **JSON, not JUnit, by design.** JUnit would require `jest-junit` on the backend. If PR-line annotations are ever needed, a JUnit reporter is added *alongside* this one, not in place of it.
