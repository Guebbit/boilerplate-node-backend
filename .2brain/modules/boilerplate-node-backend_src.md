...
1. **`src/app.ts`** — Read this first. It is the single assembly point that shows _in order_ what happens at boot: OTel → infrastructure → middleware → module mounting → lifecycle hooks. Understanding this file's sequencing makes every other file in `src/` make sense.
