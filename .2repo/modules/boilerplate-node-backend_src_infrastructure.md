...
files: 43
updated: 2026-08-31T20:51:07.395105+00:00
---
...
`src/infrastructure/` is the shared, domain-agnostic layer that every feature module in this codebase depends on. It owns the cross-cutting concerns of an HTTP JSON API—request/response shape, validation, i18n, persistence helpers, observability, and process runtime—so that individual modules (`account`, `cart`, `orders`, …) can focus purely on their own business rules without re-implementing any of those primitives.

## Key parts

- **`http/`** – The Express-facing toolkit.
  - *Entry-point helpers*: `request.ts` (single `readInput` for extracting route params / query / body), `response.ts` (canonical success/error envelope), `controller.ts` (composable read → validate → call → branch → catch helpers that stay lint-visible), `schemas.ts` (shared Zod scalars like `page`, `pageSize`), `uploads.ts` (normalised multer access), `validation-messages.ts` (global localised Zod error hook).
  - *Middleware*: `cache.ts` (Redis-backed GET caching), `locale.ts` (per-request locale via `AsyncLocalStorage`), `rate-limit.ts` + `rate-limit-store.ts` (global and per-credential limits with a shared Redis/memory store), `request-logger.ts` (one structured log line per request), `route-flag.ts` (path-segment → param normalisation).
  - *Errors*: `errors.ts` (structured HTTP error types and the single Mongoose-failure → HTTP-status mapping).

- **`i18n/`** – Request-scoped translation subsystem: `negotiate.ts` (pure `Accept-Language` matching), `context.ts` (per-request `t` via `AsyncLocalStorage`), `catalog.ts` (static dictionary loading), `overrides.ts` (DB-backed admin edits layered on top), `index.ts` (barrel).

- **`persistence/`** – Mongoose-agnostic helpers: `create-repository.ts` (generic repository factory), `serialize.ts` (Mongoose doc → wire shape), `search.ts` (pagination, regex-escaping, filter builders), `fixtures.ts` (shared test-fixture primitives), `seed.ts` (domain-agnostic upsert/export), `metrics.ts` (DB query Prometheus counters).

- **`observability/`** – Monitoring and tracing: `metrics-http.ts` (RED + process metrics), `stream.ts` (SSE live dashboard), `process-snapshot.ts` (atomic memory/uptime read), `tracer.ts` (OTel span helpers), `audit.ts` (structured audit trail), `dependency-health.ts` (readiness snapshot), `metrics-cache.ts` (cache-invalidation failure counter), `analytics/` (provider port + Umami / PostHog / no-op implementations).

- **`runtime/`** – Process bootstrap and teardown: `database.ts` (Mongoose connection lifecycle), `environment.ts` (safe int/bool env coercions), `otel-sdk.ts` (OTel SDK start), `server-lifecycle.ts` (graceful shutdown sequence).

- **`surfaces/`** – Reusable controller factories (e.g. `create-delete-controller.ts`) that produce fully-wired Express handlers for recurring CRUD patterns so modules only supply their entity-specific spec.

## How it connects

- **`src/modules/*`** (account, cart, orders, products, …) are the primary consumers. Every module's `controllers/`, `services/`, and `repository.ts` import the HTTP helpers, persistence factory, i18n context, and observability emit helpers from here. No module reaches into Express, Mongoose, or i18next directly for cross-cutting logic.
- **`src/infrastructure/adapters/`** provides the concrete storage/network adapters (Redis client, queue, etc.) that this module's cache middleware, rate-limit store, and analytics providers call into. The infrastructure layer defines *what* to store; the adapters decide *where*.
- **`src/modules/locales/`** is the sole writer for the `i18n/overrides.ts` database overlay; infrastructure reads it at boot but never writes back, keeping the dependency one-directional.
- **`db/` and `scripts/`** exercise the persistence and seed helpers (fixtures, upsert-by-id) to populate and verify the database outside the HTTP request path.
- **`tests/unit/infrastructure/`** and **`tests/cross-cutting/`** hold the unit and integration suites that lock down the contracts defined here (envelope shape, rate-limit behaviour, i18n negotiation, serializer output).

## Where to start

1. **`http/response.ts`** – Reading the canonical envelope (the `success` discriminant, error-code mapping) first gives you the single contract every endpoint honours, which makes every other file in the module easier to read.
2. **`http/request.ts`** – Understanding `readInput` and the declaration-object pattern shows how controllers actually receive their arguments, and it pairs naturally with `controller.ts` to reveal the full request-handling flow a newcomer will see in every module.
...
- `src/infrastructure/http/schemas.ts` — Defines shared Zod schemas for the small set of scalar HTTP inputs (`page`, `pageSize`, `hardDelete`) accepted by multiple endpoints. It exists so these scalars are validated once in infrastructure rather than re-derived per controller, preventing the kind of disagreement that once made `GET /products` and `GET /feedback` disagree on a legal page size. Bounds and defaults track `openapi.yaml` via orval-generated constants, with cross-cutting tests guarding drift.
...
- `src/infrastructure/persistence/fixtures.ts` — Shared fixture-building primitives that every module's `fixtures.ts` would otherwise repeat: identity handling (`_id` + pinned timestamps), a derived overrides type, and small utilities for date/ID conversion and `undefined`-stripping. Exists so module-level factories stay thin and consistent.
...
- `src/infrastructure/surfaces/create-delete-controller.ts` — Factory that produces the shared Express handler for the `DELETE /x`, `DELETE /x/:id`, and `DELETE /x/:id/hard` triplet used across every module. Each entity module supplies a small spec (entity name, service call, audit action, not-found key) and receives back a fully-wired handler—id extraction, `hardDelete` flag merging, validation, service dispatch, audit emission, and error mapping—so the cross-cutting logic lives in one place while the per-entity differences stay in the module's own file.
