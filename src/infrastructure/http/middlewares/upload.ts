 ...
 * three request handlers built on multer, filed here rather than under `adapters` because that
 * is what they are. The read side — `readUploadedImage`, which reads back what this pipeline
...
export const quarantineUploadedImages: RequestHandler = (request, _response, next) => {
    ...
    void Promise.allSettled(staged.map((stagedPath) => imageStore.quarantine(stagedPath)))
        .then(
            (results) => {
            ...
            // an ordinary thrown error would.
        )
        .catch((error: unknown) => next(error));
};
