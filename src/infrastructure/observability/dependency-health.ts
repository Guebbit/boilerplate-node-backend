 ...
 * A document store, a cache and a message broker have nothing in common technically, and every one
 * of them was previously described in its own words or not described at all. One enum makes the
 * payload readable without a legend, and makes `overallStatus` a fold rather than three special
 * cases. `connecting` is kept as its own value rather than folded into `unavailable` because the
