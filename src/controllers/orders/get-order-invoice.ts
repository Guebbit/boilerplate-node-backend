...
const getOrderInvoice = (request: Request, response: Response): Promise<void> =>
    ...
    OrderService.getById(String(request.params.id), userScope(request))
        .then((order) => {
            ...
            return Promise.all([import('ejs'), import('puppeteer-core')]).then(
                ([ejs, puppeteer]) => {
                    /**
                            ...
                            puppeteer
                                ...
                                .then((browser) =>
                                    browser
                                        ...
                                        .then((page) => page.pdf({ format: 'A4' }))
                                        // STREAMING: send data directly as bytes
        ...
        .catch((error) =>
            rejectResponse(response, 500, 'Invoice generation failed', [
