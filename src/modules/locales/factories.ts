 ...
 * pair `(locale, key)` — though both pin an id, since `insertIfAbsent`/`upsertById` key their
 * idempotency on it, not on the entry's other fields. Fields a factory doesn't state fall to
 * `./model`'s `default:`, so what gets seeded is a record of the schema, not of a factory's guess.
 */
