/**
 * @module
 * The invoice route and the order response, over real HTTP: every order carries a full VAT
 * breakdown, on every line and at the order level. `api.contract.test.ts` covers the invoice
 * route's AUTHORIZATION scope; this covers what it actually RENDERS and what `GET /orders/{id}`
 * actually publishes.
 */
...
describe('GET /orders/{id}/invoice — the VAT table', () => {
