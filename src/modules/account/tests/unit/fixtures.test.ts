/**
 * @module
 * `makeAddressBook` — the address-book fixture builder. Unlike the cart and wishlist builders,
 * this one gives every entry an `_id`: an address-book line is addressed by its own id (`PUT
 * /account/addresses/:addressId` names it), so a fixture without one seeds a book whose entries
