...
export interface IAddressesView {
    addresses: Address[];
}

const toAddress = (item: IAddressItem): Address => ({
    id: String(item._id),
...
const toView = (book: IAddressBookDocument | null): IAddressesView => ({
...
export const addressesGet = (userId: string): Promise<IAddressesView> =>
...
export const addressAdd = (
    userId: string,
    entry: Omit<IAddressItem, '_id' | 'default'> & { default?: boolean }
): Promise<IResponseSuccess<IAddressesView> | IResponseReject> =>
...
export const addressForCheckout = (
    userId: string,
    addressId?: string
): Promise<IAddressItem | null | undefined> =>
