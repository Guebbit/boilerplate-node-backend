/**
 * @module
 * `makeProduct` — the catalogue fixture builder. Not test-only: `scenarios/products.ts` seeds the
 * shipped demo catalogue through it, so a defect here reaches every seeded product. The rule it
 * holds: anything a record doesn't state is left to the SCHEMA's `default:` — `stripUndefined`
 * drops unset overrides entirely, since a key present as `undefined` blocks Mongoose's default
 * from applying.
 */
