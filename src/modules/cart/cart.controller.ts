...
/**
 * Cart controller – all routes require authentication.
 * Replaces src/controllers/cart.ts + src/routes/cart.ts.
 */
@Controller('cart')
...
export class CartController {
     ...
     * Declared before /:productId to prevent "summary" from being captured as a param.
     */
    @Get('summary')
    async getCartSummary(@Req() request: Request, @Res() response: Response): Promise<void> {
     ...
     * Declared before /:productId to prevent "checkout" from being captured as a param.
     */
    @Post('checkout')
    async checkout(@Req() request: Request, @Res() response: Response): Promise<void> {
        const user = request.user!;
     ...
     */
    @Get()
    async getCart(@Req() request: Request, @Res() response: Response): Promise<void> {
        const user = request.user!;
        const cart = await buildCartResponse(user);
     ...
     */
    @Post()
    async upsertCartItem(@Req() request: Request, @Res() response: Response): Promise<void> {
        const user = request.user!;
     ...
     */
    @Delete()
    async clearCart(@Req() request: Request, @Res() response: Response): Promise<void> {
        const user = request.user!;
     ...
     */
    @Put(':productId')
    async updateCartItemById(@Req() request: Request, @Res() response: Response): Promise<void> {
        const user = request.user!;
     ...
     */
    @Delete(':productId')
    async removeCartItem(@Req() request: Request, @Res() response: Response): Promise<void> {
        const user = request.user!;
        const { productId } = request.params;
        const productIdString = String(productId);
