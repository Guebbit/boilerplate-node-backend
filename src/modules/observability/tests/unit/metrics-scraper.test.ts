/**
 * `isMetricsScraper` — the credential guard on `GET /observability/metrics`. It is the only
 * credential check in the codebase that does not go through the JWT middleware, because
 * Prometheus cannot log in, and it protects an endpoint whose body is a map of when the service is
 * weakest. Three properties are asserted separately because each one fails silently on its own:
 *
