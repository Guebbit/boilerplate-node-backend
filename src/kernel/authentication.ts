 ...
 * kernel declares the port and `account` supplies it at boot, like `IAuditSink` and `IImageStore`.
 *
...
export interface IAuthResolver {
    fromAccessToken: (token: string) => Promise<IAuthenticatedUser | undefined>;
    fromRefreshToken: (token: string) => Promise<IAuthenticatedUser | undefined>;
}

let resolver: IAuthResolver | undefined;
 ...
 */
export const registerAuthResolver = (implementation: IAuthResolver): void => {
 ...
 */
const requireResolver = (): IAuthResolver => {
    if (!resolver)
        throw new Error(
...
export const resolveAccessToken = (token: string): Promise<IAuthenticatedUser | undefined> =>
    Promise.resolve().then(() => requireResolver().fromAccessToken(token));

/** Resolve a refresh token, for the cookie path. */
export const resolveRefreshToken = (token: string): Promise<IAuthenticatedUser | undefined> =>
