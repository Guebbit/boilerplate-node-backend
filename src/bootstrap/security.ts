 ...
 *
 * Lives outside `src/core/**` deliberately: `@middlewares/security` is application code, and the
 * `no-restricted-imports` rule in `eslint.config.ts` forbids core from reaching up to it. This is
 * the assembly layer, which is allowed to know about both.
 */
