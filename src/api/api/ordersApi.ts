...
export class OrdersApi {
     ...
     * @param email 
     */
    public async ordersList (page: number, product?: string, user?: string, email?: string, options: {headers: {[name: string]: string}} = {headers: {}}) : Promise<{ response: http.IncomingMessage; body: OrderListResponse;  }> {
        const localVarPath = this.basePath + '/orders/{page}'
        ...
        if (page === null || page === undefined) {
            throw new Error('Required parameter page was null or undefined when calling ordersList.');
        }
        ...
        return interceptorPromise.then(() => {
            ...
            return new Promise<{ response: http.IncomingMessage; body: OrderListResponse;  }>((resolve, reject) => {
                localVarRequest(localVarRequestOptions, (error, response, body) => {
                    if (error) {
                        reject(error);
                    } else {
                        if (response.statusCode && response.statusCode >= 200 && response.statusCode <= 299) {
                            body = ObjectSerializer.deserialize(body, "OrderListResponse");
